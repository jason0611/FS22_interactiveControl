...
---FUNCTION_DIRECT_PAGE_GROUP_SELECT
InteractiveFunctions.addFunction("DBL_SHOWPAGE", {
    posFunc = function(target, data, noEventSend)
        local DashboardLive = getExternalModClass("FS22_DashboardLive", "DashboardLive")

        if DashboardLive ~= nil then
            if target.spec_DashboardLive ~= nil and DashboardLive.SHOWPAGE ~= nil then
                DashboardLive.SHOWPAGE(target, data.group, data.page)
            end
        end
    end,
	schemaFunc = function(schema, path)
		schema:register(XMLValueType.INT, path .. ".dashboardLive#group", "DashboardLive page group")
		schema:register(XMLValueType.INT, path .. ".dashboardLive#page", "DashboardLive page")
	end,
	loadFunc = function(xmlFile, key, data)
		local group = xmlFile:getValue(key .. ".dashboardLive#page")
		local page = xmlFile:getValue(key .. ".dashboardLive#page")
		data.group = group or 0
		data.page = page or 0
	end,
    isEnabledFunc = function(target, data)
		local spec = target.spec_DashboardLive
		local returnValue = false
		if data.group ~= 0 and data.page ~= 0 then
			returnValue = spec ~= nil and spec.pageGroups ~= nil and spec.pageGroups[data.group] ~= nil and spec.pageGroups[data.group].pages ~= nil and spec.pageGroups[data.group].pages[data.page] ~= nil
		elseif data.group ~= 0 then
			returnValue = spec ~= nil and spec.pageGroups ~= nil and spec.pageGroups[data.group] ~= nil
		else
			returnValue = spec ~= nil and spec.pageGroups ~= nil and spec.pageGroups[1] ~= nil and spec.pageGroups[1].pages ~= nil and spec.pageGroups[1].pages[data.page] ~= nil
		end
    end
})